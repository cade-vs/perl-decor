...
sub de_check_pkg_name
{
  my $name = shift;
  
  return $name =~ /^[a-zA-Z_0-9]+(::[a-zA-Z_0-9]+)*$/o ? 1 : 0;
}

sub de_check_pkg_name_boom
{
  my $name = shift;
  my $msg  = shift || "invalid PKG NAME [$name]";
  
  de_check_pkg_name( $name ) or boom $msg;
}
